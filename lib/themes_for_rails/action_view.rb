...
module ThemesForRails
  ...
  module ActionView
    ...
    def theme_stylesheet_path(asset, new_theme_name = self.theme_name)
      base_theme_stylesheet_path(:theme => new_theme_name, :asset => digest_for_stylesheet(check_add_extension(asset, '.css'), new_theme_name))
    end

    def theme_javascript_path(asset, new_theme_name = self.theme_name)
      base_theme_javascript_path(:theme => new_theme_name, :asset => digest_for_javascript(check_add_extension(asset, '.js'), new_theme_name))
    end

    def theme_image_path(asset, new_theme_name = self.theme_name)
      base_theme_image_path(:theme => new_theme_name, :asset => digest_for_image("#{asset}", new_theme_name))
    end
